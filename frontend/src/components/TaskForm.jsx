...
const schema = yup.object({
    name: yup.string().required('Tên nhiệm vụ không được để trống.').max(100, 'Tối đa 100 ký tự'),
...
const TaskForm = ({editingTask, onAdd, onUpdate, onClose}) => {
  ...
  function colorPriority(priority){
        return priority === 'High' ? 'danger' : (priority==='Low' ? 'success' : 'warning');
    }
      ...
      <div className="modal-dialog " style={{ maxWidth: '500px' }}>
        <form onSubmit={handleSubmit(onSubmit)} >
          <div className="modal-content no-radius border border-2 ">
            ...
            {/* Header */}
            <div className="modal-header bg-light text-black"  style={{ borderBottom: 'none' }}>
              <h5 className="modal-title">{editingTask ? 'Sửa nhiệm vụ' : 'Thêm nhiệm vụ'}</h5>
            ...
            <div className="modal-body">
                ...
                <div className="mb-3">
                    <label className="form-label text-muted">Tiêu đề</label>
                    <input type="text" className={getInputClass('name')} {...register('name')} />
                ...
                <div className="mb-3">
                    ...
                    <div className='d-flex align-content-center gap-3'>
                      {['High', 'Medium', 'Low'].map((value, index) => (
                        <div className='d-flex' key={index}>
                            ...
                            />
                            <label className={`btn btn-${value} btn-outline-${colorPriority(value)}`} htmlFor={`priority${value}`}>
                                {value}
                            </label>
            ...
            <div className="modal-footer" style={{ borderTop: 'none' }}>
              ...
              <button type="submit" className="btn btn-primary">{editingTask ? 'Cập nhật' : 'Thêm'}</button>
            </div>
