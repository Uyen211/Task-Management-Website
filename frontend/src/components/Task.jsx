...
const Task = () => {
    ...
    async function handleUpdate(task, alert){
        ...
        if(alert=='yes') showAlert("Cập nhật nhiệm vụ thành công!", 'success');
        const newEditTask = resJson.task;
        setTasks(tasks.map((t) => (t._id === newEditTask._id ? newEditTask : t)));
    ...
    return(
        <div className='container-fluid m-0 p-0'>
            ...
            {alert.message && (
              <p className={`alert alert-${alert.type} ${visible ? 'visible' : ''} m-4 `} id='alert'>
                {alert.message}
              </p>
            )}
